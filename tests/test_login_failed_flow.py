...
def test_login_failed_flow(driver, username, password, expected_message):
    ...
    assert (
        login_error_page.is_error_message_displayed()
    ), "Error message is not displayed"
    # verify error message text
    assert (
        expected_message in login_error_page.get_error_message()
    ), f"Expected error message to contain '{expected_message}' but got '{actual_error_message}'"
